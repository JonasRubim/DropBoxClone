class JMStorageController {
  constructor() {
    this.currentFolder = ["JSMStorage"];  
    ...
    this.btnFolderEl = document.querySelector("#btn-new-folder");
  ...
  connectFirebase() {  // conectando o firebase
    var firebaseConfig = {
      apiKey: "AIzaSyARWBrWpIJhTE8mH0NKpof4kTxOpt5MU2Y",
      authDomain: "jmstorage-5a044.firebaseapp.com",
      databaseURL: "https://jmstorage-5a044-default-rtdb.firebaseio.com",
      projectId: "jmstorage-5a044",
      storageBucket: "jmstorage-5a044.appspot.com",
      messagingSenderId: "30182828785",
      appId: "1:30182828785:web:2a7b13d3bd9e51df00d750",
      measurementId: "G-LLC5ZQHWHG",
    };
    // Initialize Firebase
    firebase.initializeApp(firebaseConfig);
  }

  getSelection() { // selecionando todos os li selecionado
    
    return this.listFilesEl.querySelectorAll(".selected")
  }

  removeFolderTask(ref, name) { // criando metodo para remover pasta 
  ...
  removeTask() {  // criando metodo excluir....
  ...
  initEvents() {  // iniciando eventos

    this.btnFolderEl.addEventListener("click", e => {
    ...
    this.btnDeleteEl.addEventListener("click", e => { // botão excluir 
          
      this.removeTask().then(resp => {
    ...
    this.btnRenameEl.addEventListener("click", e => {
        // renomeando arquivo e atualizando o firebase
    ...
    this.btnSendFileEl.addEventListener("click", (event) => {
      // click no button *enviar arquivos*
      this.inputFileEl.click();
    });

    this.inputFileEl.addEventListener('change', event=>{

      this.uploadTask(event.target.files).then(resp =>{
      
      this.getFirebaseRef().push().set({
                          name: resp[0].name,
                          path: resp[0].customMetadata.url,
                          type: resp[0].contentType,
                          size: resp[0].size
      })
  ...
  uploadComplete() {   // desativando o modal e limpando os valores do input
    this.modalShow(false);
    this.inputFileEl.value = "";
    this.btnSendFileEl.disable = false;
  }

  getFirebaseRef(path) {   // metodo para adicionar dados no firebase
  ...
  modalShow(show = true) {  // ativa e desativar modal
    this.snackModalEl.style.display = show ? "block" : "none";
  }


  // ajax(url, method= "GET", formData = new FormData(), onprogress = ()=>{}, onloadStart = ()=> {} ) { // criando o metodo ajax para ser usado em outras partes do codigo...
  //  return  new Promise((resolve, reject) => {
  ...
  uploadTask(files) {   // tratando os arquivos recebido
  ...
  // Calulando o tempo de barra do upload
  uploadProgress(event, file) {

    let timeSpent = Date.now() - this.startUploadTime;
    console.log("Fim " + timeSpent)
    console.log("inicio" + this.startUploadTime)
  ...
  formTimeToHuman(duration) {  // formatando horas do modal
  ...
  getFileIconView(file){   //adiciona o icone correto no determinado tipo de arquivo
  ...
  getFileView(file, key) {   // metodo para atribuir li aos arquivos recebidos 
  ...
  readFiles() {   // adicionando arquivos na tela 
     
    this.lastFolder = this.currentFolder.join("/"); // pegar a ultima pasta
  ...
  initLiEvent(li) { // adicionando evento de click no elemento li
